...
plot_data <- threats %>% 
  ...
  mutate(years_extinct = 2021 - as.numeric(extinct),
         rev_years_extinct = years_extinct * -1,
  ...
  distinct(binomial_name, continent, years_extinct, rev_years_extinct) %>% 
...
p <- ggplot(plot_data)+
  ...
  geom_segment(aes(y = binomial_name, yend = binomial_name, x = rev_years_extinct, xend = years_extinct, colour = continent), size = 1.5) +
  theme_bw()+
